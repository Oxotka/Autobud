...
Процедура ВыгрузитьОстатки(Команда)
	...
	ЗаписатьЭлементXML(Запись, "SENDER",       SENDER);
	ЗаписатьЭлементXML(Запись, "RECIPIENT",    RECIPIENT);
#Область Position
	...
	Для Каждого Стр Из ОстаткиНоменклатуры Цикл
		...
		ЗаписатьЭлементXML(Запись, "POSITIONNUMBER",    Строка(ПорядковыйНомер));
		ЗаписатьЭлементXML(Запись, "PRODUCT",           Стр.ШтрихКод);
		...
		ЗаписатьЭлементXML(Запись, "STOCKQUANTITY",     Строка(Стр.Остаток));
		ЗаписатьЭлементXML(Запись, "PRICE",             Формат(Стр.Цена, "ЧДЦ=2; ЧС="));
		ЗаписатьЭлементXML(Запись, "NETPRICE",          Формат(Стр.ЦенаБезНДС, "ЧДЦ=2; ЧС="));
		ЗаписатьЭлементXML(Запись, "VAT",               Строка(Стр.СтавкаНДСЧислом));
