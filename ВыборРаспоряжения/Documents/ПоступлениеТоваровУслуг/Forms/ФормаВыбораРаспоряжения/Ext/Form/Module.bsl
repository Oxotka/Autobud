...
&НаСервере
Процедура ВыборРаспоряжения_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	СписокРаспоряжений.Сортировать("Дата");
КонецПроцедуры

&НаКлиенте
Процедура ВыборРаспоряжения_ПоказатьВсеПриИзмененииПосле(Элемент)
	СписокРаспоряжений.Сортировать("Дата");
КонецПроцедуры

&НаКлиенте
Процедура ВыборРаспоряжения_ОтборДатаПриИзмененииПосле(Элемент)
	Если ЗначениеЗаполнено(ОтборДата) Тогда
		ЗаполнитьСписокРаспоряженийПоДате();
	Иначе
...
&НаСервере
Процедура ЗаполнитьСписокРаспоряженийПоДате()
	...
	|							И КонецПериода(ЗаказПоставщику.Дата, День) = &Дата
	|							И (Склад = &Склад
	...
	Запрос.УстановитьПараметр("Дата",						КонецДня(ОтборДата));
