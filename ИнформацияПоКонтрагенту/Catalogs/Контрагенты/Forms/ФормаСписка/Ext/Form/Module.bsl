...
&НаСервере
Процедура КУТ_ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	
	КУТ_СвернутьРазвернутьПанель(СвернутьПанель);
...
Процедура КУТ_ПриАктивизацииСтроки()
	...
	КУТ_РазместитьДанныеКонтрагента(ТекущиеДанные.Ссылка);
	
КонецПроцедуры

&НаСервере
Процедура КУТ_РазместитьДанныеКонтрагента(Знач Контрагент)
	
	ДанныеКонтрагента = КУТ_ДанныеДляЗаполненияПоКонтрагенту(Контрагент);
	
	Элементы.ДекорацияНаименованиеПолное.Заголовок = ДанныеКонтрагента.НаименованиеПолное;
	Элементы.ДекорацияДолгНам.Заголовок = КУТ_НадписьВзаиморасчетов(ДанныеКонтрагента.ДолгНам, "Дт");
	Элементы.ДекорацияДолгНаш.Заголовок = КУТ_НадписьВзаиморасчетов(ДанныеКонтрагента.ДолгНаш, "Кт");
...
&НаСервере
Процедура КУТ_СвернутьРазвернутьПанель(СвернутьПанель)
...
Функция КУТ_ДанныеДляЗаполненияПоКонтрагенту(Знач Контрагент)
	...
	|	РасчетыСКлиентами.СуммаОстаток КАК ДолгКонечныйОстаток,
	...
	|	""Поставщик""
	|ИЗ
	|	РегистрНакопления.РасчетыСПоставщиками.Остатки(, ) КАК РасчетыСПоставщиками
	|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК РегистрАналитикаУчетаПоПартнерам
...
&НаКлиенте
Процедура КУТ_ДекорацияРазвернутьНажатие(Элемент)
...
&НаКлиенте
Процедура КУТ_ДекорацияСвернутьНажатие(Элемент)
