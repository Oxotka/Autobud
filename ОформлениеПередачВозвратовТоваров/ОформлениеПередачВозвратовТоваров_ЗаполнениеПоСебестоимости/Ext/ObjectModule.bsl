...
Процедура ЗаполнитьЦеныПоСебестоимости(Док)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров.Остатки(&Период, ) КАК СебестоимостьТоваровОстатки
	...
	|			ТОГДА ЕСТЬNULL(ВТ_Себестоимость.СтоимостьОстаток, 0)
	|		ИНАЧЕ ЕСТЬNULL(ВТ_Себестоимость.СтоимостьОстаток, 0) / ВТ_Себестоимость.КоличествоОстаток
