...
Процедура Номенклатура_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+ Добавлено Н.И. Исправление ошибки замены текста запроса
...
|	СпрНоменклатура.АБ_СнятоСПроизводства,
...
|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СвободныеОстатки.Остатки(, Склад В ИЕРАРХИИ (&ОтборСклад)) КАК СвободныеОстаткиОстатки
...
|				Сегменты.Сегмент = &СегментНоменклатуры)) КАК Поле2}";
	...
	РедактированиеПоступления = ПравоДоступа("Редактирование", Метаданные.Документы.ПриобретениеТоваровУслуг);
...
&НаСервере
Процедура ПолучитьДополненныйЗапросДинамическогоСписка(ТекстЗапроса)
	...
	|	СпрНоменклатура.АБ_СнятоСПроизводства КАК АБ_СнятоСПроизводства,
	|	ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток, 0) + ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток, 0) КАК ВРезерве
	|ИЗ
	|	Справочник.Номенклатура КАК СпрНоменклатура
	|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(, Склад В ИЕРАРХИИ (&ОтборСклад)) КАК ТоварыНаСкладахОстатки
...
Процедура Номенклатура_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если РаботаСНоменклатурой.ПравоИзмененияДанных() Тогда 
		...
		ПолучитьДополненныйЗапросДинамическогоСписка(СвойстваСписка.ТекстЗапроса);
...
Процедура Подключаемый_ВыполнитьКомандуОтчет(Команда)
	...
	ТекущиеДанные = ТекущиеДанныеСписка();
	//МенюОтчетыУТКлиент.ВыполнитьПодключаемуюКомандуОтчет(Команда, ЭтотОбъект, ТекущиеДанные);
...
Функция НазваниеДокумента(ИмяДокумента)
	...
	СоответствиеИменДокументов.Вставить("ПоступлениеТоваровУслуг", НСтр("ru='Приобретение товаров и услуг'"));
