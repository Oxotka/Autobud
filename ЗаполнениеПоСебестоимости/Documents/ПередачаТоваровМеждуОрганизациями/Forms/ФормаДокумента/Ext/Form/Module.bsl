...
Процедура Себестоимость_ЗаполнитьЦеныПоСебестоимостиПередНаСервере()
	...
	Запрос.УстановитьПараметр("ВидЗапасов", Справочники.ВидыЗапасов.ВидЗапасовДляПередачиМеждуОрганизациямиПоУмолчанию(Объект.Организация, ОБъект.ОрганизацияПолучатель));
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Товары.НомерСтроки,
	|	Товары.Номенклатура,
