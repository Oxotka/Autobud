...
&НаКлиенте
Процедура АдресСклада_СоглашениеПриИзмененииПосле(Элемент)
	
	АдресСклада_СоглашениеПриИзмененииПослеНаСервере();
...
Процедура ДополнитьСписокВыбораАдресов()
	...
	Если ЭлементАдреса.СписокВыбора.НайтиПоЗначению(СтруктураАдреса.Значение) = Неопределено Тогда
...
&НаСервере
Процедура АдресСклада_СоглашениеПриИзмененииПослеНаСервере()
