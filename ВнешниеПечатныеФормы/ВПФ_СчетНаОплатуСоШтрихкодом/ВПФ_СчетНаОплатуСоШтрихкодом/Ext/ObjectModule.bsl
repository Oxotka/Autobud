...
Процедура Печать(МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетНаОплатуШтрихкод") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			СформироватьПечатнуюФормуСчетНаОплату(СтруктураТипов, СписокОбъектов)
			);
	КонецЕсли;
...
Функция СформироватьПечатнуюФормуСчетНаОплату(СтруктураТипов, ОбъектыПечати, КомплектыПечати = Неопределено) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
				...
				ДобавитьШтрихкод(ОбластьСтрокаТаблицы, СтрокаТовары.Штрихкод, КоличествоМиллиметровВПикселе);
...
Процедура ДобавитьШтрихкод(Область, ЗначениеШтрихкода, КоличествоМиллиметровВПикселе)
	
	Для каждого Рисунок Из Область.Рисунки Цикл
		Если НЕ СтрНачинаетсяС(Рисунок.Имя, "ШтрихкодНоменклатуры") Тогда
			Продолжить;
		КонецЕсли;
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов)
	...
	|	МАКСИМУМ(&ТекстЗапросаНаименованиеЕдиницыИзмерения) КАК ЕдиницаИзмерения,
	|	Товары.КоличествоУпаковок КАК Количество,
	|	Товары.Цена КАК Цена,
	|	Товары.Сумма КАК Сумма,
	|	Товары.СтавкаНДС КАК СтавкаНДС,
	|	Товары.СуммаНДС КАК СуммаНДС,
	|	МАКСИМУМ(НЕОПРЕДЕЛЕНО) КАК ВидЦеныИсполнителя,
